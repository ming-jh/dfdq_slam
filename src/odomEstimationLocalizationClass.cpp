// Author of DFDQ_SLAM: Ming JiHua 
// Email 1533768175@qq.com
...
void OdomEstimationClass::matchPointsToMap(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& edge_in, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& surf_in){
    ...
    for (int iterCount = 0; iterCount < optimization_count; iterCount++){
        ceres::LossFunction *loss_function = new ceres::HuberLoss(0.1);//Construction of kernel functions
        ceres::Problem::Options problem_options;//Declaration
        ceres::Problem problem(problem_options);//First declare a ceres::Problem::Options, and then initialize the problem with Options

        //自定义参数类表达SE3，PoseSE3Parameterization继承自LocalParameterization
        problem.AddParameterBlock(parameters, 7, new PoseSE3Parameterization());//Call AddResidualBlock to add the error item to the target function，
        ...
        ceres::Solver::Options options;//Configure solver parameters
...
bool OdomEstimationClass::addEdgeCostFactor(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& pc_in, ceres::Problem& problem, ceres::LossFunction *loss_function){
    ...
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        pointAssociateToMap(&(pc_in->points[i]), &point_temp);//Convert the current frame to the world coordinate 
